...
import { ZIPF_SOURCES, zipfCurve, ENTROPY_BY_LAG, ENTROPY_COLORS } from '../lib/data'
...
function ZipfChart({ active, hovered, setHovered }: {
  ...
  const line = d3.line<{ rank: number; freq: number }>()
    .x(d => x(d.rank))
    .y(d => y(Math.max(0.6, d.freq)))
