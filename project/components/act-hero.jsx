/* ACT 0 — HERO
   Opening: bioluminescent particles drift; "Sounding" mark glows;
   a sonar ring pulses at the heart of the page. */

function ActHero() {
  // Subtle parallax on the sonar
  ...
  // Drifting particles overlay (lightweight DOM-driven, atop the SVG bg)
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      dots.forEach(d => {
        const cx = parseFloat(d.el.style.left);
        const cy = parseFloat(d.el.style.top);
        let nx = cx + d.dx;
        let ny = cy + d.dy;
        ...
        d.el.style.left = nx + '%';
        d.el.style.top  = ny + '%';
        d.el.style.opacity = (0.25 + Math.abs(Math.sin(t + d.twink)) * 0.6).toFixed(2);
      });
  ...
  return (
    <section id="hero" className="act act-hero" data-screen-label="01 Surface">
      ...
      <div className="col">
        <Eyebrow><span style={{ color:'var(--lumen)' }}>An interactive essay</span> · ocean acoustics, communication, and the long attempt to listen</Eyebrow>
        <h1>
          Sounding<br/>
